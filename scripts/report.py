...
LANG_T = {
    "zh": {
        ...
        "gmv_dd": "- GMV 组合 max_dd = **{v:.2%}**（最小方差 ≠ 最小回撤：GMV 重仓低波动商品 DBC/EEM，回撤反而大于切线组合）",
        "s3": "## 3. 切线组合最优权重",
        "s3_head": "| 资产 | 类别 | 权重 |",
        "s3_sep": "|---|---|---|",
        "s3_cat": "**按资产类别汇总权重：**",
        ...
        "s6_2": "2. **负相关价值**：EEM-DBC 相关系数 -0.12（新兴市场股票 vs 商品），组合内负相关资产对冲单类资产风险；",
    ...
    "en": {
        ...
        "gmv_dd": "- GMV portfolio max_dd = **{v:.2%}** (min-variance ≠ min-drawdown: GMV overweights low-vol commodity DBC/EEM, so its drawdown exceeds tangency)",
        ...
        "s6_2": "2. **Value of negative correlation**: EEM-DBC correlation is -0.12 (emerging-market equities vs commodities); negatively correlated assets hedge single-class risk within the portfolio;",
