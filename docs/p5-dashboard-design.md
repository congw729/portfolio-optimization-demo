...
| 展示 | plotly heatmap（或 imshow）：6×6 相关系数矩阵，**悬停显示数值**；**负相关对（GLD-DBC = -0.12）红框/星标标注**，并加 caption「负相关 = 分散化增量来源」 |
...
| 教学点 | 强调「方案 A 含负相关（GLD-DBC），SPY-TLT 实测 +0.68 正相关（近 5 年股债同向）」——v1.2 实测口径，作为全场信息量最大单图 |
