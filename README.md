...
## 5-Agent Cluster Orchestration

The core of the demo is a **5-agent + 1-leader** cluster pipeline running on jiuwenswarm team mode (task board + `send_message` + a shared `.team/` workspace) that completes the full chain: **data collection → feature engineering → portfolio optimization → visualization → reporting**. The leader only breaks down and verifies tasks — **writes no code**; five teammates claim tasks and hand off along `blocked_by` dependencies, and agents forward only the **artifact path + a one-line summary**, never the raw data.

### Role Duties

| Role | Task | Duty | Input → Output | Script |
|---|---|---|---|---|
| leader | — | Breaks down & verifies tasks (no code) | design → 5 tasks + acceptance verdicts | none (orchestration only) |
| data-collector | T1 | Fetch + clean + cache (cache-first; `--refresh` re-fetches) | network/cache → `data/params.json`, `data/returns_*.csv` | `scripts/fetch_data.py` |
| feature-engineer | T2 | Derived features (corr / class summary / 60-40 benchmark) | `data/params.json` + returns → `data/features.json` | `scripts/features.py` |
| optimizer-engine | T3 | GMV / tangency / efficient frontier (no-short primary + analytic contrast) | params + returns + features → `output/portfolios.csv`, `frontier.csv` | `scripts/optimizer.py` |
| viz-agent | T4 | 5 charts + dashboard | frontier/portfolios → `output/*.png` | `scripts/viz.py` |
| reporter | T5 | Aggregates metrics & conclusions | all artifacts → `output/report.md` | `scripts/report.py` |

### Dependency Chain (DAG)

```
leader (breaks down & verifies, writes no code)
   │ creates T1–T5 tasks (blocked_by expresses dependencies)
   ▼
data-collector ──► feature-engineer ──► optimizer-engine ──► viz-agent ──► reporter
                                                  └───────────────────────────►
                              (reporter also depends on optimizer-engine: T3 → T5)
```
...
| T3 optimizer-engine | T2 (strict) | consumes `data/features.json` (pre-checks negative correlation & reads 60/40 params) |
| T4 viz-agent | T3 | needs frontier / portfolios ready |
| T5 reporter | T3 + T4 | needs optimization results & charts ready |

### Message Flow (agents forward only "path + summary")

| Step | Sender → Receiver | Message (illustrative) |
|---|---|---|
| M1 | data-collector → feature-engineer | `data/params.json ready (incl. asset_class) — please compute features` |
| M2 | feature-engineer → optimizer-engine | `data/features.json ready (corr / 60-40 benchmark) — please optimize` |
| M3 | optimizer-engine → viz-agent | `output/frontier.csv + portfolios.csv ready — please chart` |
| M4 | optimizer-engine → reporter | `output/portfolios.csv ready (GMV/tangency); frontier_stocks.csv is the baseline` |
| M5 | viz-agent → reporter | `output/*.png 5 charts generated (incl. two-frontier comparison) — start final aggregation` |
| M6 | reporter → team-leader | `output/report.md ready (Sharpe / drawdown / class weights / 60-40) — demo deliverable` |

> Teaching point: every hand-off shows "agents forward only path + summary, never the data" — the engineering value of context decoupling and file-based collaboration in cluster mode.

> Full design: `docs/p4-orchestration-design.md` (§1 role definitions, §2 DAG, §3 message flow).
...
├── case-studies/             # Bug case studies (teaching material)
│   └── dashboard-badcases.md # dashboard bugs: symptom → root cause → lesson → fix
